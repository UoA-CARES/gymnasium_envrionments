...
import torch
import train_loops.crucial_path_policy_loop_episode_reward_till_high as cpbe
...
def main():
    ...
    iterations_folder = f"{alg_config.algorithm}-{env_config.task}-{datetime.now().strftime('%y_%m_%d_%H-%M-%S')}-e_reward_till_h"
