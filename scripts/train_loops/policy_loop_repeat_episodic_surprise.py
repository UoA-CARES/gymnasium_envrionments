...
def policy_based_train(
    ...
    for total_step_counter in range(int(max_steps_training)):
        ...
        if done or truncated:
            ...
            (not crucial_steps and memory.long_term_memory.is_full() and episode_surprise > memory.long_term_memory.get_min_reward() and episode_timesteps > 2):
                    print(f"addddddddddddddddddd episode num:{episode_num}, episode surprise:{episode_surprise}, min_reward:{memory.long_term_memory.get_min_reward()}")
                   ...
                   # print(f"total_reward:{total_reward}, min_reward:{memory.long_term_memory.get_min_reward()}")
                    ...
                    # input()
                    memory.long_term_memory.add([episode_num,episode_surprise, states, actions,rewards, next_states, dones, episode_nums, episode_steps])
