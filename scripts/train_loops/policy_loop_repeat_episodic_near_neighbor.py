    ...
    for total_step_counter in range(int(max_steps_training)):
        ...
        if total_step_counter < max_steps_exploration or explore:
            ...
            if episode_timesteps == 1:
                crucial_actions,crucial_states, crucial_episode_num, episode_steps,crucial_rewards,crucial_total_reward= memory.vem_goal.fetch_high_rewards_nearest_episode(current_state=state)
        ...
        if (total_step_counter +1) % RF == 0 and memory.vem_goal.get_length()>0:#and total_step_counter >10000:
        ...
        if done or truncated:
            ...
            if number_of_crucial_episodes == 1:
             ...
             # 1. when try to find episode with higher episode reward    # and 
            elif ((not memory.vem_goal.is_full() and not crucial_steps and episode_reward > 0) or 
                  (not crucial_steps and memory.vem_goal.is_full() and 
                  episode_reward > memory.vem_goal.get_min_reward() and episode_timesteps > 2)):
                    print(f"addddddddddddddddddd episode num:{episode_num}, episode reward:{episode_reward}, min_reward:{memory.vem_goal.get_min_reward()}")
                   ...
                   # print(f"total_reward:{total_reward}, min_reward:{memory.long_term_memory.get_min_reward()}")
                    ...
                    # input()
                    memory.vem_goal.add([episode_num,episode_reward, states[0], states, actions,rewards, next_states, dones, episode_nums, episode_steps])
