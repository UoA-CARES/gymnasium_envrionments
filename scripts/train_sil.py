...
import torch
import train_loops.policy_loop_sil as pbsil
...
def main():
    ...
    iterations_folder = f"{alg_config.algorithm}-{env_config.task}-{datetime.now().strftime('%y_%m_%d_%H-%M-%S')}-SIL"
    ...
    for training_iteration, seed in enumerate(training_config.seeds):
       ...
       #     ppe.ppo_train(env, agent, record, training_config, alg_config)
        if agent.type == "policy":
            ...
            elif alg_config.algorithm in ["TD3SIL", "SACSIL"]:
                pbsil.policy_based_train(
                    env, agent, memory, record, training_config, alg_config
                )
            else:
