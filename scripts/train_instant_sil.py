...
import torch
import train_loops.policy_loop_instant_SIL as pis
...
def main():
    ...
    iterations_folder = f"{alg_config.algorithm}-{env_config.task}-{datetime.now().strftime('%y_%m_%d_%H-%M-%S')}-instant_SIL_RN5"
    ...
    for training_iteration, seed in enumerate(training_config.seeds):
       ...
       #     ppe.ppo_train(env, agent, record, training_config, alg_config)
        if agent.type == "policy":
            ...
            elif  alg_config.algorithm in ["ReTD3SIL" ,"RESACSIL"]:
                pis.policy_based_train(
                    env, agent, memory, record, training_config, alg_config
                )
            else:
