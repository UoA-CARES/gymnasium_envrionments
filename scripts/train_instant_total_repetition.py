...
import torch
import train_loops.policy_loop_repeat_instant_total_reward as pr
...
def main():
    ...
    iterations_folder = f"{alg_config.algorithm}-{env_config.task}-{datetime.now().strftime('%y_%m_%d_%H-%M-%S')}-instant_total_reward_RN4"
